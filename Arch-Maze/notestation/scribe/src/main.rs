...
struct Config {
    ...
    hash: bool, // --hash: content-hash the selection (SHA-256), the "verified" tier
}
...
impl App {
    ...
    fn build_plan_json(&self, shas: Option<&HashMap<String, String>>) -> String {
    ...
    fn commit_write(&mut self) {
        ...
        let plan = self.build_plan_json(shas.as_ref());
    ...
    fn simulate(&mut self) {
        ...
        let plan = self.build_plan_json(None);
...
fn print_help() {
    println!(
        "scribe {VERSION} — scan, heatmap, select-to-backup, with a simulation hook\n\n\
         ...
         --hash        verified tier: SHA-256 sign every selected file on write\n\
